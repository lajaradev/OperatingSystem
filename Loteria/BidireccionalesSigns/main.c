...
int CombinacionGanadoraGenerada = 0; // Variable que permitira a los procesos hijos saber si la combinación ganadora ha sido generada.
HIJO *arrayHijos;
...
int main(int argc, char *argv[]){
	...
	char *numSorteo = argv[1]; // Cantidad de sorteos a realizar.
	int numJugadores = atoi(argv[2]); // Cantidad de juadores en cada sorteo.
	int *CombinacionGanadora = malloc(6 * (sizeof(int))); // Memoria para almacenar la combinacion ganadora.
	int i, j;

	// Se define la estructura para el manejador de las señales que recibiran tanto padre como hijo.
	...
	/**** HANDLER HIJOS ****/
	// Se define la estructura para el manejador de las señales que recibe el hijo	 
	// La señal que recibirá el hijo será para indicar que la combinacion ganadora ya ha sido generada
	manejador.sa_sigaction = SorteoRealizado;// Nombre de la funcion manejadora
	manejador.sa_flags = SA_SIGINFO; // Esto lo definimos para enviar informacion adicional junto a la señal
	sigaction(SIGUSR1, &manejador , NULL); // Establecemos el manejador de la señal a la función manejadora (es importante indicar el nombre para las señales hijo, SIGUSR1)
	/**** FIN HANDLER HIJOS ****/
		...
		if(pid < 0){
	   	...
	   	if(pid == 0){ // CODIGO DEL HIJO
    		...
    		// Espera el aviso del padre de que la combinación ganadora ha sido generada
	   		while (CombinacionGanadoraGenerada == 0){
	   			pause();
	   		}
			...
			// Asignamos los datos a sus hijos
	...
	for(i = 0; i < numJugadores; i ++){
		IndicarQueHayGanadora(arrayHijos[i].pid);
	}
