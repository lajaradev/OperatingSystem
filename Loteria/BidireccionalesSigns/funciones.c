...
extern int CombinacionGanadoraGenerada;
...
void SorteoRealizado(int sig, siginfo_t *siginfo, void *context){
	...
	// Esta función es el manejador de la señal que reciben los hijos para enterarse que el padre ya ha generado la combinación ganadora, es decir, que se ha realizado el sorteo. 
 	// Enviamos el ACK al hijo.
 	CombinacionGanadoraGenerada = CombinacionGanadoraGenerada + 1;
}
...
void IndicarQueHayGanadora(int pid){

	// Esta funcion será usada por el proceso padre para enviar una señal a los procesos hijos.
	// Dicha señal les permitirá saber que la combinación ganadora ha sido generada.
	...
	value.sival_int = getpid(); 
 	...
 	// Usamos kill para enviar una señal al proceso hijo
	kill(pid, SIGUSR1);
